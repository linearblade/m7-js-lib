# Usage Documentation - Table of Contents

[README](../../README.md) -> [Usage TOC](./TOC.md) -> [Architecture Index](../architecture/INDEX.md) -> [API Index](../api/INDEX.md)

This section contains practical user-facing guidance for integrating and using `m7-js-lib`.

If you are new, read top-to-bottom.

---

## Getting Started

* **Introduction** -> [INTRODUCTION.md](./INTRODUCTION.md)
* **About m7-js-lib** -> [../ABOUT.md](../ABOUT.md)
* **Quick Start** -> [QUICKSTART.md](./QUICKSTART.md)
* **Installation** -> [INSTALLATION.md](./INSTALLATION.md)
* **Requirements** -> [REQUIREMENTS.md](./REQUIREMENTS.md)

---

## Core Usage Contracts

* **Entrypoint Contract (`index.js`, `auto.js`)** -> [../entrypoints-contract.md](../entrypoints-contract.md)
* **Philosophy** -> [../PHILOSOPHY.md](../PHILOSOPHY.md)
* **What Makes m7-js-lib Different** -> [../WHAT_MAKES_US_DIFFERENT.md](../WHAT_MAKES_US_DIFFERENT.md)

---

## API and Architecture

* **API Index** -> [../api/INDEX.md](../api/INDEX.md)
* **LLM API Contract** -> [../api/M7_JS_LIB_API_CONTRACT.md](../api/M7_JS_LIB_API_CONTRACT.md)
* **Architecture Index** -> [../architecture/INDEX.md](../architecture/INDEX.md)

---

## Related

* **Use Policy** -> [../USE_POLICY.md](../USE_POLICY.md)
* **AI Disclosure** -> [../AI_DISCLOSURE.md](../AI_DISCLOSURE.md)
* **Project README** -> [../../README.md](../../README.md)
